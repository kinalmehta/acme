...
def main(_):
  ...
  if RUN_DISTRIBUTED.value:
    program = experiments.make_distributed_experiment(
        ...
        num_actors=2,
        inference_server_config=inference_server.InferenceServerConfig(batch_size=2, 
          update_period=1, 
          timeout=datetime.timedelta(seconds=1, milliseconds=0, microseconds=0)),
        )
    lp.launch(program, launch_type="local_mp", 
    local_resources={
      "learner":PythonProcess(env={"CUDA_VISIBLE_DEVICES": str(0)}),
